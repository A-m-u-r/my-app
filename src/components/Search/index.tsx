...
export const SearchTowns: React.FC<ISearchTownProps> = (props) => {
  ...
  let filteredTowns = useMemo(() => TOWNS.filter(([ru_name, en_name]) => ru_name.toLowerCase().includes(query.toLowerCase())).slice(0, 10), [query])
  return (
    <Combobox value={props.town} onChange={props.setTown}>
      ...
      <Combobox.Options>
        {filteredTowns.map(([ru_name, en_name]) => (
          <Combobox.Option key={ru_name} value={en_name.toLowerCase().slice(0, en_name.indexOf(",")).trim()}>
            {ru_name}
          </Combobox.Option>
        ))}
      </Combobox.Options>
...
//       > 
//         {TOWNS.map(([ru_name, en_name]) => (
//           <option
//             key={en_name}
//             value={en_name.toLowerCase().slice(0, en_name.indexOf(","))}
//           >
//             {ru_name}
//           </option>
//         ))}
